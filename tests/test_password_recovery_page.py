...
class TestPasswordRecovery:
    ...
    def test_password_recovery_brings_to_recovery_page(self, driver):
        ...
        entrance_page.open_page(subdir=Constants.ENTRANCE_PAGE_SUBDIRECTORY)
        ...
        assert password_recovery_page.check_page(subdir=Constants.RECOVER_PASSWORD_SUBDIRECTORY)
    ...
    def test_password_reset_for_valid_email(self, driver, make_user, create_user_payload):
        ...
        password_recovery_page = PasswordRecoveryPage(driver)
        password_recovery_page.open_page(subdir=Constants.RECOVER_PASSWORD_SUBDIRECTORY)
        password_recovery_page.wait_for_recovery_page_header_loaded()
        email = payload["email"]
        ...
        assert reset_password_page.check_page(subdir=Constants.RECOVER_PASSWORD_SUBDIRECTORY)
    ...
    def test_password_reset_gets_highlighted(self, driver, make_user, create_user_payload):
        password_recovery_page = PasswordRecoveryPage(driver)
        password_recovery_page.open_page(Constants.RECOVER_PASSWORD_SUBDIRECTORY)
        password_recovery_page.wait_for_recovery_page_header_loaded()
        payload = create_user_payload(name='rand', password='rand', email='rand')
        user = make_user(data=payload)
        email = payload["email"]
        ...
        reset_password_page = PasswordResetPage(driver)
        reset_password_page.wait_for_recovery_page_header_loaded()
        reset_password_page.check_page(subdir=Constants.RESET_PASSWORD_SUBDIRECTORY)
